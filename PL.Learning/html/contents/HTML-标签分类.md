...
- [1. 结构性标签](#1-结构性标签)
  - [1.1 div 快级元素](#11-div-块级元素)
  - [1.2 html5语义标签](#12-html5语义标签)
    - [1.2.1 main](#121-main)
    - [1.2.2 header](#122-header)
    - [1.2.3 nav](#123-nav)
    - [1.2.4 article](#124-article)
    - [1.2.5 section](#125-section)
    - [1.2.6 footer](#126-footer)
    - [1.2.7 figure](#127-figurefigcaption)
    - [1.2.8 address](#128-address)
    - [1.2.9 aside](#129-aside)
    - [1.2.10 hgroup](#1210-hgroup)
    - [1.2.11 menu](#1211-menu)
- [2. 段落和文本标签](#2-段落和文本标签)
  - [2.1 标题标签](#21-标题标签)
  - [2.2 段落、换行、水平线标签](#22-段落换行水平线标签)
    - [2.2.1 hr标签](#211-hr标签)
  - [2.3 文字修饰标签](#23-文字修饰标签)
  - [2.4 不再推荐的标签](#24-不再推荐的标签)
  - [2.5 code、kbd、samp、var](#25-codekbdsampvar)
  - [2.6 bdi、bdo、wbr](#26-bdibdowbr)
- [3. 媒体元素标签](#3-媒体元素标签)
  - [3.1 img](#31-img-标签)
  - [3.2 picture](#32-picture-标签)
  - [3.3 figure、figcaption](#33-figurefigcaption-标签)
  - [3.4 vedio](#34-vedio)
  - [3.5 track](#35-track)
  - [3.6 audio](#36-audio)
  - [3.7 embed](#37-embed)
  - [3.8 source](#38-source-标签)
- [4. 表格标签](#4-表格标签)
  - [4.1 table](#41-table)
  - [4.2 caption](#42-caption-标签)
  - [4.3 thead tbody tfoot](#43-theadtbodytfoot)
  - [4.4 col](#44-col-标签)
  - [4.5 colgroup](#45-colgroup)
  - [4.6 tr](#46-tr-标签)
  - [4.7 th](#47-th-标签)
- [5. HTML5新增表单标签](#5-html5新增表单标签)
  - [5.1 datalist](#51-datalist)
  - [5.2 progress](#52-progress)
  - [5.3 fieldset ](#53-fieldset)
- [6. 链接标签](#6-链接标签)
  - [6.1 a 标签](#61-a-标签)
  - [6.2 map 标签](#62-map-标签)
  - [6.3 area 标签](#63-area-标签)
  - [6.4 base 标签](#64-base-标签)
- [7. 面板标签](#7-面板标签)
  - [7.1 details](#71-details)
  - [7.2 dialog](#72-dialog)
- [8. 列表标签](#8-列表标签)
  - [8.1 dl、dt、dd](#81-dldtdd)
  - [8.2 ol li 有序列表](#82-ol-li)
  - [8.3 ul li 无序列表](#83-ul-li)
- [9. 功能标签](#9-功能标签)
  - [9.1 meter](#91-meter)
  - [9.2 time](#92-time)
...
结构性标签 (Structural Tags): 定义网页的结构和内容的组织方式。它们勾勒出文档的骨架，并赋予不同部分语义化的含义。使页面内容更有逻辑性和条理性，方便浏览器和搜索引擎理解网页的结构，也利于屏幕阅读器等辅助技术更好地解读内容，提升可访问性。

#### 1.1 div 块级元素

块级元素是指本身属性为`display:block;` 的元素。因为它自身的特点，我们通常使用块级元素来进行大布局（大结构）的搭建。
...
`<div>(这个元素包含全局属性)` HTML 元素是流式内容的通用容器。`<div>` 是一个**块级元素**，用于划分网页中的区域，通常用于布局。。除非以某种方式使用 CSS 对其进行样式设置（例如，直接应用样式，或者对其父元素应用某种布局模型，如弹性盒子），否则它对内容或布局没有影响。
...
- `<header>`：定义页面或区域的头部内容，通常包含导航链接、标志、标题等，等同`div`。
  - `<nav>`：定义导航链接的部分等同`div`。
- `<main>`：定义文档主体内容的主要部分，通常是页面中独一无二的主要内容,等同`div`。
  - `<article>`：定义独立的内容块，适用于博客文章、新闻报道等,等同`div`。
  - `<section>`：定义文档中的某个部分，通常包含一组主题相关的内容,等同`div`。
- `<aside>`：定义与页面内容相关的附属内容，通常用于侧边栏或广告等,等同`div`。。
- `<footer>`：定义页面或区域的底部内容，通常包含版权信息、联系信息等,等同`div`。
...
<p>
  Victor Frankenstein, a Swiss scientist, has a great ambition: 
  to create intelligent life.  But when his creature first stirs,
  he realizes he has made a monster. A monster which, abandoned by 
  his master and shunned by everyone who sees it, follows Dr Frankenstein to
  the very ends of the earth.
</p>
```

##### [1.2.11 menu](#)
**HTML5 不支持 `<frame>` 标签**, HTML 元素在 HTML 规范中被描述为 `<ul>` 的语义替代，但浏览器将其视为与 `<ul>` 没有区别（并通过无障碍树暴露）,它表示一个无序列表（由 `<li>` 元素表示）。
...
|br|换行标签|`<br>` 元素在文本中生成一个换行（回车）符号|
|hr|水平线标签|语义为一个文字段落|

```html
<hgroup>
  <h1>中央第十五巡视组巡视民政部党组工作动员会召开</h1>
  <hr>
  <p>
    根据党中央关于巡视工作的统一部署，近日，中央第十五巡视组巡视民政部党组工作动员会召开。<br>
    中央第十五巡视组组长刘军川作了动员讲话，对深入学习贯彻习近平总书记关于巡视工作的重要讲话精神，
    扎实开展巡视工作提出要求。民政部党组书记、部长陆治原主持会议并讲话。
  </p>
</hgroup>
```
...
该元素包含全局属性:
- align `已弃用 非标准` 设置页面上分割线的对齐方式。如果没有指定任何值，默认值为 left。
- color `非标准 非标准` 使用颜色名或十六进制设置分割线的颜色。
- noshade `已弃用 非标准` 令分割线不包含阴影,默认值是有。
- size `已弃用 非标准` 设置分割线高度的像素值。
- width `已弃用 非标准` 使用像素或者百分比设置分割线的宽度。
...
主要介绍一下em、strong、small、span、del、ins、sub、sup、mark标签。

|标签|介绍|备注|
|:-------|:----------|:-----------|
|em|文字修饰，行内元素|着重阅读，元素将文本标记为强调（emphasis）格式。效果为**斜体**|
|strong|文字修饰，行内元素|十分重要的内容，效果为**加粗** `可以使用css实现（更推荐）`|
|small|文字修饰，行内元素|十分重要的内容，效果为**变细小** `可以使用css实现（更推荐）`|
|**span**|文字修饰，行内元素|没有语义，通用包谷内容的容器|
|del|文字修饰，行内元素|没有语义，文件增加中划线 `可以使用css实现（更推荐）`|
|ins|文字修饰，行内元素|没有语义，文件增加下划线 `可以使用css实现（更推荐）`|
|sub|文字修饰，行内元素|下标内容 水为：H<sub>2</sub>O|
|sup|文字修饰，行内元素|上标内容 一共要2 <sup>12</sup> 升水。|
|mark|文字修饰，行内元素 | 表示为引用或符号目的而标记或突出显示的文本，这是由于标记的段落在封闭上下文中的相关性或重要性造成的。`可以使用css实现（更推荐）`|
...
#### [2.4 不再推荐的标签](#)
一下元素都没有什么意义，也不语义化，都应该使用css来实现。

| 标签 | 介绍        |备注|
|:---|:----------|:------------|
|s| 文字修饰，行内元素| 使用删除线来渲染文本。`可以使用css实现（更推荐）` |
|b| 文字修饰，行内元素 | 仍然以粗体显示文本。 `可以使用css实现（更推荐）` |
|i| 文字修饰，行内元素 | `italic` 仍然以斜体显示文本。`可以使用css实现（更推荐）` |
|u| 文字修饰，行内元素 | `underline` 默认情况下渲染为一个实线下划线。`可以使用css实现（更推荐）` |

HTML `<s>` 元素 使用删除线来渲染文本。使用 `<s>` 元素来表示不再相关，或者不再准确的事情。但是当表示文档编辑时，不提倡使用 `<s>` ；为此，提倡使用 `<del>` 和 `<ins>` 元素。

```html
<s>Today's Special: Salmon</s> <del>SOLD OUT</del><br>
```

**显示效果**：<s>Today's Special: Salmon</s><del>SOLD OUT</del><br>

HTML `<b>` 元素用于吸引读者注意元素内容，而这些内容本身并不具有特别重要性。它以前被称为粗体元素，大多数浏览器仍然以粗体显示文本,应该使用 CSS font-weight 属性。
...
##### 2.6.1 bdi 标签
中用于双向文本隔离的标签。它的全称是 "Bidirectional Isolation"。

具体来说，当你在一个方向为从左到右（LTR）的文本中插入一个方向为从右到左（RTL）的文本（例如阿拉伯语或希伯来语）时，如果不使用 `<bdi>` 标签，可能会导致混乱的显示。例如，标点符号和数字的位置可能会出现错位。使用 `<bdi>` 标签可以确保插入的文本在文档中按照其自然的书写方向显示，而不会影响周围的内容。

##### 2.6.2 bdo标签
会覆盖文本的当前方向性，以便其中的文本以不同的方向呈现。

**dir属性** 文本在此元素内容中渲染的方向。可能的值有：
* ltr：指示文本应从左到右绘制。
* rtl：指示文本应从右到左绘制。
...
##### 2.6.3 wbr 标签
标签在 HTML 中用于在文本中添加可能的换行机会。它的全称是 "Word Break Opportunity"。
...
|dfn|文字修饰，行内元素|表示术语的一个定义。`<dfn>` 元素有一个 title属性，那么该术语的值就是该属性的值。|
...
- **title** 属性是可选的，它包含一个描述图片的文本，鼠标悬停就显示。
...
- **loading** `重要` 指示浏览器应当如何加载该图像。允许的值：
...
[**JS事件属性**](https://developer.mozilla.org/zh-CN/docs/Web/HTML/Element/video#事件)
...
- kind 定义了 text track 应该如何使用。如果省略了该属性，默认的 kind 值就是 subtitles。下面是允许的关键字：
  ...
  - label 当列出可用的 text tracks 时，给浏览器使用的 text track 的标题，这种标题是用户可读的，一般备注语言类型：English、中文、日文等等。
...
<video controls poster="/images/sample.gif">
   ...
   <track kind="captions" src="sampleCaptions.vtt" srclang="en" label="English">
   <track kind="descriptions"
     src="sampleDescriptions.vtt" srclang="en" label="English=Ameircan">
   <track kind="chapters" src="sampleChapters.vtt" srclang="en" label="Spanish">
...
#### [4.2 caption 标签](#)
`<caption>` HTML 元素定义了表格的标题。元素 (or HTML 表格标题元素) 展示一个表格的标题，它常常作为 `<table>` 的第一个子元素出现，同时显示在表格内容的最前面，但是，它同样可以被 CSS 样式化，所以，它同样可以出现在任何相对于表格的做任意位置。

align	HTML5 不支持。HTML 4.01 已废弃。 定义标题的对齐方式。
- left
- right
- top
- bottom
...
rel属性：
- `noopener` 防止新页面控制原页面（安全）
- `noreferrer` 防止泄露来源页面信息（隐私）
- `nofollow` 告诉搜索引擎不要传递权重（SEO 场景）
...
- type 该属性指定在一个 MIME 类型链接目标的形式的媒体类型。
