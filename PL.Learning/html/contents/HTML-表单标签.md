...
### [1. form 标签](#)
HTML **form** 元素表示文档中的一个区域，此区域包含交互控件，用于向Web服务器提交信息，**HTML 表单用于收集用户的输入信息**。
...
- **character_set** 表单提交时使用的字符编码列表，多个字符编码使用空格分隔。
  - 值为 `UTF-8`、`UTF-16`、`ISO-8859-1`等
- **novalidate** `布尔值属性`，如果使用该属性，则提交表单时不进行验证。
...
|onchange|script function|当元素改变时运行脚本|
|oncontextmenu|script function|当触发上下文菜单时运行脚本|
|onfocus|script function|当元素获得焦点时运行脚本|
|onformchange|script function|当表单改变时运行脚本|
|onforminput|script function|当表单获得用户输入时运行脚本|
|oninput|script function|当元素获得用户输入时运行脚本|
|oninvalid|script function|当元素无效时运行脚本|
|onreset|script function|当表单重置时运行脚本。HTML 5 不支持。|
|onselect|script function|当选取元素时运行脚本|
|onsubmit|script function|当提交表单时运行脚本|
|onclick|script function|当点击时时运行脚本|
...
#### [2.3 input:image](#)
定义图像输入的替代文本，请使用img标签, `无任何意义，千万别用`。
...
```html
<label for="username">Username:</label>
<input id="username" type="image" placeholder="请输入姓名" value="默认值">
```
