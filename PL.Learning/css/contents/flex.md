...
| [align-content](https://developer.mozilla.org/zh-CN/docs/Web/CSS/align-content)  |CSS3 |无| 属性定义了多根轴线的对齐方式。如果项目只有一根轴线，该属性不起作用(`有多列时才起作用`)。|
...
|[gap](https://developer.mozilla.org/zh-CN/docs/Web/CSS/Reference/Properties/gap)|CSS3|无|gap 是 row-gap 和 column-gap 的简写属性，语法如下：|
...
<img src="./static/imagessss.png" alt="image" style="width: 700px;">
...
<img src="./static/fc7b9126-443b-444f-bdc1-1ac1a9eecbff.png" alt="image" style="width: 500px;">
...
|space-evenly|每项都沿着主轴均匀分布在指定的对齐容器中。 |
...
gap 是 row-gap 和 column-gap 的简写属性，语法如下：

```css
.container {
  display: flex; /* 启用 Flex 布局 */
  gap: <row-gap> <column-gap>; /* 第一个值为行间距，第二个为列间距（可选） */
}
```

- 若只写一个值，则 行间距和列间距相等（如 gap: 20px 表示行、列间距均为 20px）；
- 若写两个值，则分别对应 row-gap（垂直方向间距）和 column-gap（水平方向间距）。

**与 margin 的区别**

|特性|gap|margin（给子元素设置）|
|:--------------|:---------------|:-----|
|边缘间距|不影响容器|边缘子元素的 margin-left/margin-right 会导致容器边缘溢出（需容器设置 padding 抵消）|
|换行场景|换行后自动适配行间距|最后一个项目的 margin-right 或最后一行的 margin-bottom 会产生多余间距|
|计算复杂度|无需考虑边缘情况，自动均匀分配|需手动计算 n-1 个间距（n 为项目数量）|

**推荐**：优先使用 gap 控制项目间距，减少布局计算成本。