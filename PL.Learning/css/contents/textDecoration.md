...
| [text-decoration-skip `不兼容`](https://developer.mozilla.org/zh-CN/docs/Web/CSS/text-decoration-skip)           | CSS3    |有| 检索或设置对象中的文本装饰线条必须略过内容中的哪些部分。          |
...
- text-decoration-line 设置使用的装饰类型，例如 underline 或 line-through。
- text-decoration-color 设置装饰的颜色。
- text-decoration-style 设置装饰的线条的样式，例如 solid、wavy 或 dashed。
- text-decoration-thickness 设置装饰的线条粗细。 `自2021年3月起，此特性已在主流浏览器中得到支持`

```css
p{
    text-decoration: green wavy underline  0.4rem;
}
```
