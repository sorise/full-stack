...
|**grid-area**| 指定网格元素的名称，或者也可以是 `grid-row-start`, `grid-column-start`, `grid-row-end` 等的简写属性|
|**grid-template-areas**|指定如何显示行和列，使用命名的网格元素|
|[**gap**](https://developer.mozilla.org/zh-CN/docs/Web/CSS/Reference/Properties/gap)|row-gap 和 column-gap 的简写属性|
|`column-gap`|指定列之间的间隙|
|`row-gap`| 指定两个行之间的间距|
|[**grid-gap**,`gap别名`](https://developer.mozilla.org/zh-CN/docs/Web/CSS/Reference/Properties/gap)| grid-row-gap 和 grid-column-gap 的简写属性|
|`grid-row-gap`|指定网格元素的行间距|
|`grid-column-gap`|指定网格元素的间距大小|
|[grid-auto-flow](https://developer.mozilla.org/zh-CN/docs/Web/CSS/Reference/Properties/grid-auto-flow)	| 指定自动布局算法怎样运作，精确指定在网格中被自动布局的元素怎样排列。|
|[grid-auto-columns](https://developer.mozilla.org/zh-CN/docs/Web/CSS/Reference/Properties/grid-auto-columns)|指的默认的列尺寸|
|[grid-auto-rows](https://developer.mozilla.org/zh-CN/docs/Web/CSS/Reference/Properties/grid-auto-rows)	| 指的默认的行尺寸|
```css
.container{
...
### [6. Grid 间距 gap](#)
相比于margin更加优秀，可以`避免外边距折叠`、`收尾元素多余边距`的问题，可以创建整齐划一的栅格系统。

> **注意**：规范的早期版本将该属性命名为 grid-gap，且为了保持与旧网站的兼容性，浏览器仍然会接受 grid-gap 作为 gap 的别名。
...
|`column-gap`|指定列之间的间隙|
|`row-gap`| 指定两个行之间的间距|
|**grid-gap** `gap别名`| grid-row-gap 和 grid-column-gap 的简写属性|
|`grid-row-gap`|指定网格元素的行间距|
|`grid-column-gap`|指定网格元素的间距大小|


<img src="./static/gap548952465asdasimage.png" alt="image" style="width: 450px;">
...
#### [7.1 容器级对齐](#)
当网格的总尺寸小于其容器尺寸时，`容器有额外空间时`，控制整个网格在容器内的位置.

应对如下的情况：容器大，但是网格布局系统小的情况。
```css
.container{
    display: grid;
    padding: 5px;
    ...
    /* 定义 2行3列*/
    grid-template-columns:100px 200px 50px; /* 总共才350px */
    grid-template-rows:80px 80px;/* 总共才 160px */
}
```

<img src="./static/gridcontainerduiqi11image.png" alt="image" style="width: 350px;">

|选项| 说明|对象|
|:---|:------|-------|
|justify-content| 所有栅格在容器中的**水平对齐方式**，`容器有额外空间时`|栅格容器|
|align-content|所有栅格在容器中的**垂直对齐方式**，`容器有额外空间时`|栅格容器|
...
#### [7.2 项目级对齐](#)
控制单个网格项目在其分配到的单元格内部的位置，当其被分配的尺寸大于了项目本身的尺寸时，设置元素所在的位置。
...
#### [8.2 auto-fit 和 auto-fill](#)
自动填充是根据容器尺寸，自动设置元素尺寸。

这两个关键字都会在可用空间内尽可能多地创建指定尺寸的轨道，但处理"剩余空间"的方式不同。
...
[auto-fill 和 auto-fit 关键字](https://juejin.cn/post/6844903927243079693)
...
.box {
    ...
    /*grid-template-columns: 10vw 1fr 20vw;*/ 1fr其实也是一样的
    border: 5px solid #3333;
}
```
...
支持多个值,可以传入多个值，它们会依次循环应用于隐式创建的行。

```css
.container {
  display: grid;
  /* 隐式行的高度将是 100px, 50px, 100px, 50px, ... 循环 */
